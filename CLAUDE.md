...
- **2026-05-20** — Operations Dashboard mini-map and Routing page map were showing Google's default "Oops! Something went wrong / This page didn't load Google Maps correctly" overlay inside the map div when the Maps API key was rejected (referrer restriction / billing / invalid key). Added `window.gm_authFailure` handlers in [src/pages/employeeDashboard.ts](src/pages/employeeDashboard.ts) and [src/pages/routing.ts](src/pages/routing.ts) so the existing in-house fallback UI (pickup list / "map optional" message) renders instead of Google's ugly overlay. Underlying key issue still needs Cloudflare-side attention. Deployed to Cloudflare Pages (`reuse-canada-scale` project, main branch) via OAuth wrangler login — confirmed in [feedback_wrangler_auth] that `CLOUDFLARE_API_TOKEN` env var on this machine is IP-restricted and must be scrubbed with `env -u CLOUDFLARE_API_TOKEN` for wrangler to use the working OAuth creds. GitHub push of commit `993b0d1` still blocked (only read-only deploy keys available locally).
- **2026-05-20** — User asked for a "create new customer / employee login" feature on the employee side; the full functionality already existed at `/employee/customers` ("New Customer") and `/employee/drivers` ("Create Driver / Staff Account") with their corresponding `POST /api/employee/customers` (admin+manager) and `POST /api/employee/staff` (admin) endpoints. Per their pick, added a unified entry point: a new "Create Account" quick-action button on the Operations Dashboard (visible to admin+manager via client-side `localStorage.rc_session.role` check; the "New Staff/Driver" row inside the chooser is hidden for non-admins to avoid a backend 403 dead-end). The chooser deep-links to the existing pages with `?new=1`, which auto-opens their existing create modals — zero duplicated form HTML or backend changes. Non-obvious: Tailwind's `hidden` and `flex` utility classes collide and source-order on the CDN build makes `flex` win, so the quick-actions container starts hidden via inline `style="display:none;"` and toggles to `flex` in JS. Files: [src/pages/employeeDashboard.ts](src/pages/employeeDashboard.ts), [src/pages/customerManagement.ts](src/pages/customerManagement.ts), [src/pages/driverManagement.ts](src/pages/driverManagement.ts).
- **2026-05-22** — User installed an Epson TM-T88VI (model M338A) receipt printer connected to their Mac via USB and asked to wire scale-ticket printing through it. The print button + auto-print already existed but the path was designed for a *networked Star Micronics* printer (`StarWebPRNT` POST to a printer IP) and would only fall back to `window.print()` after a doomed HTTP attempt. Reality: a USB Epson on macOS is owned by CUPS — browsers cannot talk to it directly, so the only viable path is `window.print()` → macOS print dialog → OS default printer. Ripped out the Star/IP code path entirely (`printReceiptToThermal` now just calls `browserPrintReceipt`; dropped `printerConnected`/`printerIP`/`connectPrinter`/`formatReceiptText`/`escapeXml` and the saved-IP bootstrap), replaced the misleading "Printer IP" sidebar input with a status card explaining the OS default-printer flow + a "Test Print" button (`printTestReceipt()` renders a sample with `ticket_number = 'TEST-PRINT'`), tightened the 80mm `@media print` CSS (added `@page { size: 80mm auto; margin: 0 }`, 76mm content, monospace Menlo fallback, 8mm trailing feed so the auto-cutter doesn't slice the "Thank you" line), and rewrote `browserPrintReceipt`'s HTML using flex print-rows instead of `<table>`s. Also dropped `autoPrintReceipt`'s double `printReceiptToThermal(receipt)` call — Chrome collapses back-to-back `window.print()` invocations into a single dialog, so it never actually produced 2 copies; operator now sets copies in the macOS dialog. **Important context for future Claude**: the user pointed at the blue "Done" button in the Assign Customer modal (`submitAssignment()`, [scaleHouse.ts:563](src/pages/scaleHouse.ts#L563)) as where print should fire, but that button only *assigns* customer + material on a weighed-in ticket — the ticket isn't completed until the truck weighs out via the "Use live scale weight" button (`useLiveTareInAssignModal()` → `/merge-out` → `autoPrintReceipt`) or the orange merge-confirm flow (`confirmMerge()` → `autoPrintReceipt`). Both completion paths already trigger auto-print; no new wiring needed at "Done". Files: [src/pages/scaleHouse.ts](src/pages/scaleHouse.ts).

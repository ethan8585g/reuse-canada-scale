...
export function renderCraneTickets(): string {
  return layout('Crane Tickets', employeePageWrapper('crane-tickets', 'Digital Crane Tickets', `
    <!-- Action Bar -->
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
      ...
      <button onclick="openNewTicketModal()" class="bg-rc-orange hover:bg-rc-orange-light text-white font-semibold py-2.5 px-5 rounded-lg shadow-sm hover:shadow-md btn-press transition-all duration-200 flex items-center gap-2">
        <i class="fas fa-plus"></i> New Crane Ticket
      </button>
    </div>
    ...
    <div id="new-ticket-modal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 hidden items-center justify-center p-4" style="display:none;">
      <div class="bg-white rounded-2xl shadow-modal modal-enter w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
          <h3 class="text-lg font-bold text-gray-800"><i class="fas fa-weight mr-2 text-rc-orange"></i>New Crane Ticket</h3>
        ...
        <div class="p-6">
          <div class="mb-4">
            ...
            <button type="button" id="pull-bridge-btn" onclick="pullWeightFromBridge()"
              ...
              <span id="pull-bridge-label">Pull from Overhead Crane</span>
            </button>
            <p id="pull-bridge-status" class="text-xs text-center mt-2 text-gray-400">
              <i class="fas fa-info-circle mr-1"></i>Pulls the live reading from the connected crane terminal
            </p>
          </div>
    ...
    <script>
      ...
      function openWeightModal(ticketId, type, ticketNumber) {
        ...
        document.getElementById('pull-bridge-status').innerHTML = '<i class="fas fa-info-circle mr-1"></i>Pulls the live reading from the connected crane terminal';
      ...
      async function refreshBridgePreview() {
        ...
        labelEl.textContent = 'Pull from Overhead Crane';
        btn.disabled = false;
        try {
          const res = await axios.get('/api/crane-bridge/current');
          const d = res.data || {};
          if (d.fresh && d.weight_kg > 0) {
            labelEl.textContent = 'Pull from Overhead Crane (' + Number(d.weight_kg).toLocaleString('en-CA', {maximumFractionDigits:1}) + ' kg)';
          } else {
            labelEl.textContent = 'Overhead Crane — no live reading';
            btn.disabled = true;
          }
      ...
      async function pullWeightFromBridge() {
        ...
        try {
          ...
          if (!d.fresh) {
            ...
            statusEl.innerHTML = '<i class="fas fa-exclamation-triangle mr-1"></i>Overhead Crane terminal is not publishing live weight. Ask them to connect the indicator, or enter the weight manually.';
            return;
          }
          if (!(d.weight_kg > 0)) {
            statusEl.className = 'text-xs text-center mt-2 text-amber-600 font-semibold';
            statusEl.innerHTML = '<i class="fas fa-exclamation-circle mr-1"></i>Indicator shows 0 kg — place the load on the hook.';
            return;
          }
      ...
      async function submitWeight() {
        ...
        try {
          await axios.post('/api/crane-tickets/' + currentWeightTicketId + '/weight', {
      ...
      async function createTicket(e) {
        e.preventDefault();
        try {
          await axios.post('/api/crane-tickets', {
            customer_id: parseInt(document.getElementById('ticket-customer').value),
            material_type: document.getElementById('ticket-tire-type').value,
      ...
      async function submitVoid() {
        ...
        try {
          await axios.post('/api/crane-tickets/' + id + '/void', { reason });
            ...
            <div class="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <div class="space-y-2 text-sm">
                  ...
                  <div class="flex justify-between"><span class="text-gray-500">Material</span><span class="capitalize">\${escHtml((t.material_type || 'N/A').replace('_',' '))}</span></div>
